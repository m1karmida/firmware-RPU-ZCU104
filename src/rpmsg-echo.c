	...
	//initialization
		...
		while(1){
				...
				if (control == EVENT_SUSPEND){
					LPRINTF("[SS] suspended task resumed %d\n",totalRuntime);
...
void startTasks(int index,TickType_t wcet, int suspend ){
	...
	if (!suspend){
		...
		LPRINTF("[SS] aperiodic task started. \n");
	} else{
...
void timerTS(void * pvParameters){
	...
	for (int i = 0; i < RPU_N_TASK && HigherPriorityTaskEx == 0 ; i++){
		...
		if (*(Task_vector[i].Handler)!= NULL && eTaskGetState(*(Task_vector[i].Handler)) == eReady && Task_vector[i].priority >  uxTaskPriorityGet(ssTask))
	...
	if (RA > 0)
		LPRINTF("[TIMERTS] capacity CS recharged of %d ticks. CS = %d\n",RA,CS);
	...
	if (runningTask!= NULL){
		...
		if (remainingExTime > 0){
	    	...
	    	if (CS > 0){
				...
				totalRuntime += runtime;
				LPRINTF("[TIMERCSOLD] Timer expired, capacity all consumed.\n");
				suspendedTask = 1;
